...
class JoyTeleop:
    def __init__(self):
        ...
        self.pub_AlbertVel = rospy.Publisher('Albert_desired_vel', Twist, queue_size=10)
    ...
    def user_jetson(self, joy_data):
        ...
        :jetson joy_data:
            ...
            右摇杆按下: buttons[14]
        '''
        # print ("user joy_data.buttons: ", joy_data.buttons)
        ...
        if joy_data.buttons[0] == joy_data.buttons[1] == joy_data.buttons[
            ...
            if joy_data.buttons[1] == 1:  # B
                self.autorun = 0
                self.autoturnR = not self.autoturnR
                self.autoturnL = 0
            if joy_data.buttons[3] == 1:  # X
                self.autorun = 0
                self.autoturnL = not self.autoturnL
                self.autoturnR = 0
        ...
        if self.autorun == True:
            xlinear_speed = 0.2 * self.xspeed_limit * self.linear_Gear
        else:
            xlinear_speed = self.filter_data(joy_data.axes[1]) * self.xspeed_limit * self.linear_Gear

        if self.autoturnR == True:
            angular_speed = -0.2 * self.angular_speed_limit * self.angular_Gear
        elif self.autoturnL == True:
            angular_speed = 0.2 * self.angular_speed_limit * self.angular_Gear
        else:
        ...
        self.pub_AlbertVel.publish(twist)
